...
def make_structured_output_text(*, candidate_id: str | None = None) -> str:
    return json.dumps(
        {
            "answer": (
                "Project Atlas is owned by the Platform Intelligence team. "
            ...
            "citations": [
                {
                    "candidate_id": (
                        candidate_id or "11111111-1111-1111-1111-111111111111"
                    ),
                    "evidence_span": (
                        "Project Atlas is owned by the Platform Intelligence team."
                    ),
...
def test_openai_llm_provider_generates_answer_from_responses_api() -> None:
    fake_response = SimpleNamespace(
        output_text=make_structured_output_text(),
    ...
    assert response.citations[0].candidate_id == (
        "11111111-1111-1111-1111-111111111111"
    )
    assert response.citations[0].evidence_span == (
        "Project Atlas is owned by the Platform Intelligence team."
    )
    ...
    assert "candidate_id" in call["instructions"]
    assert "evidence_span" in call["instructions"]
    assert "Who owns Project Atlas?" in call["input"]
    assert "candidate_id=11111111-1111-1111-1111-111111111111" in call["input"]
...
def test_openai_llm_provider_returns_zero_usage_when_usage_is_missing() -> None:
    fake_response = SimpleNamespace(
        output_text=json.dumps(
            {
                "answer": "The context does not contain enough information.",
                "citations": [],
            },
        ),
        usage=None,
...
def test_openai_llm_provider_rejects_invalid_json_output() -> None:
    fake_response = SimpleNamespace(
    ...
    with pytest.raises(
        LLMProviderError,
        match="not valid JSON for the generation contract",
    ):
        provider.generate_answer(make_request())
