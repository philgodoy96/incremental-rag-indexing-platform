...
def test_ingest_local_seed_documents_returns_ingestion_result() -> None:
    ...
    assert payload["vector_entries_updated"] == 0
    assert payload["vector_entries_deactivated"] == 0
    ...
    assert payload["documents"][0]["vector_entries_updated"] == 0
    assert payload["documents"][0]["vector_entries_deactivated"] == 0
